...
type RequestHandler<T> = (request: T) => Promise<any>;
...
handlers.set('call_tool', async (request: any) => {
  const { name, arguments: args } = request;

  switch (name) {
    case 'run_web_tests': {
      const browser = (args as any).browser || 'chromium';
      const headed = (args as any).headed || false;
      const response = `Running Playwright web tests on ${browser}${headed ? ' (headed)' : ''}... Tests execution initiated.`;
      return { content: [{ type: 'text', text: response }] };
    }

    case 'run_api_tests': {
      const endpoint = (args as any).endpoint;
